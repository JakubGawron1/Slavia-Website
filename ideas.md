...
~~2671. **FE — Podpowiedzi tekstowe dla ról (Role Tooltips in Admin Panel)** — Małe, estetyczne podpowiedzi (Tooltips) wyjaśniające uprawnienia poszczególnych ról przy edycji konta w panelu AdminsManager.~~ *(wdrożone: UTooltip przy odznakach ról w AdminsManager.)*
...
## Bank pomysłów 10001+ (Super Ulepszenia Platformy)

10001. **Zaawansowany Silnik Synchronizacji Offline-First (CRDT) [Slavia-frontend]** - Zmiana paradygmatu zarządzania stanem na pełny tryb offline-first przy użyciu IndexedDB (lub PWA) oraz mechanizmów CRDT (Conflict-free Replicated Data Type). Pozwala to na nieprzerwane korzystanie z panelu trenera i dziennika zawodnika nawet w podziemnych salach treningowych bez zasięgu sieciowego. Gdy zasięg powraca, Service Worker w tle automatycznie synchronizuje kolejki zapytań (wpisy wyników, obecności), dbając o bezbłędne łączenie równoległych modyfikacji.
10002. **Przetwarzanie Wideo Zero-Copy i Live Streaming [Slavia-backend]** - Przebudowa potoku obsługi multimediów w Rust z użyciem bibliotek typu `ffmpeg-next` i tzw. buforów zero-copy. Gdy zawodnik wgrywa wideo do analizy, strumień jest od razu transkodowany w locie do formatu HLS (HTTP Live Streaming) lub DASH. Zapobiega to blokowaniu pamięci RAM na serwerze i drastycznie przyspiesza moment odtwarzania wideo w aplikacji na frontendzie – trenerzy mogą odtwarzać film od razu, bez oczekiwania na zakończenie pełnego transkodowania.
10003. **Lokalne AI – On-Device Pose Estimation [Slavia-mobile]** - Wdrożenie lekkich modeli uczenia maszynowego (np. TensorFlow Lite / MoveNet) bezpośrednio do aplikacji Flutter, pozwalających na analizę toru sztangi w czasie rzeczywistym przy użyciu wyłącznie aparatu urządzenia. Cała kalkulacja (wektory siły, prędkość sztangi dla Velocity Based Training) dzieje się na smartfonie bez konieczności uploadu ciężkich plików wideo na serwer. To zapewnia maksymalną oszczędność danych, ochronę prywatności zawodników oraz zero opóźnień.
10004. **Zintegrowany Asystent Głosowy (STT) Trenera [Slavia-mobile]** - Zastosowanie mechanizmów Speech-to-Text na urządzeniu (on-device), co umożliwia dyktowanie wyników zawodników podczas dynamicznej sesji bez odrywania oczu od pomostu. Trener po prostu mówi: „Janek, rwanie 120 kilo, udane, technika dobra”, a skrypt analizuje frazę wyłapując słowa-klucze i automatycznie uzupełnia oraz zatwierdza formularz dziennika w odpowiedniej karcie zawodnika.
10005. **Event-Sourced Architecture dla Logów Audytowych i Finansów [Slavia-backend]** - Zastosowanie wzorca Event Sourcing dla ścieżek krytycznych platformy (składki, płatności, oficjalne wyniki decydujące o rankingu Sinclaira). Zamiast "nadpisywać" dane w bazie SQLite poprzez UPDATE, każda akcja biznesowa staje się trwałym, niezmiennym zdarzeniem (np. `AthleteScoreUpdatedEvent`). Pozwala to na pełną historyczną odtwarzalność, idealne śledzenie edycji "kto, kiedy i co zmienił" oraz błyskawiczne rozwiązywanie konfliktów.
10006. **Kinetogram 3D – Interaktywna Przeglądarka Kinezjologiczna [Slavia-frontend]** - Zaawansowany widok oparty na WebGL / Three.js, który przekształca dane z analizy sztangi i sylwetki z dwuwymiarowego wideo w abstrakcyjny model 3D punktów węzłowych. Trener może obracać perspektywę (oglądać wyrzut bioder z dowolnego kąta), powiększać wektory siły układające się przestrzennie wokół zawodnika, co drastycznie ułatwia wyłapywanie błędów we wczesnej fazie pociągnięcia sztangi, które są niewidoczne z profilu.
10007. **Dynamiczny Generator Obciążeń z Opcją Auto-Regulacji (AI) [Slavia-backend]** - Algorytm obliczeniowy, który przed wygenerowaniem planu treningowego na dany dzień analizuje historię zawodnika, jego sen i parametry RPE. Jeśli system wykryje kumulację zmęczenia na osi czasu, dyskretnie przelicza i obniża tonaż na dany trening o 5-10% (deload), ostrzegając trenera. W przypadku szczytowej formy sugeruje ostrzejsze podejścia pod rekordy (PR), co daje w pełni inteligentne sterowanie mikrcyklem.
10008. **WebRTC Live Coaching (Telemetria na Żywo) [Slavia-frontend]** - Implementacja technologii WebRTC dla połączeń Peer-to-Peer bezpośrednio z poziomu przeglądarki i aplikacji. Trener przebywający na wyjeździe może na żywo obserwować swojego podopiecznego trenującego w klubie przy minimalnym opóźnieniu (sub-100ms). Ekran udostępnia interfejs tablicy – trener rysuje po strumieniu wideo kąty dociągnięcia i linie toru sztangi, które natychmiast pokazują się zawodnikowi na jego smartfonie (nakładka AR).
10009. **Dedykowana sub-aplikacja WearOS / watchOS [Slavia-mobile]** - Mały, ultra-wysoce responsywny „companion app” instalowany na smartwatchach. Posiada kluczowe funkcje asystujące przy boju: wielkie stoper odpoczynku pomiędzy seriami, logowanie tętna (peak heart rate) po zrzuceniu sztangi oraz potężną haptykę, która przypomina że czas przerwy minął bez konieczności odblokowywania telefonu czy brudzenia ekranu magnezją.
10010. **Smart Caching Layer na Bazie Redis / Moka [Slavia-backend]** - Integracja wysokowydajnej pamięci podręcznej typu LFU (Least Frequently Used) w architekturze Rusta. Ma to kluczowe znaczenie w czasie dni z zawodami, kiedy kibice, zawodnicy i trenerzy nieustannie odświeżają widoki rankingów (szczególnie złożonych zapytań dla kalkulacji Sinclaira). Cache izoluje plik główny SQLite przed zablokowaniem (lock-contention), dostarczając zapytania z pamięci w czasie <1ms.
10011. **Live Leaderboard (Wyzwania Aktualizowane via Sockets) [Slavia-frontend]** - Zaprzęgnięcie protokołu WebSockets do strumieniowania modyfikacji wyników na żywo. Podczas ogłoszonych wyzwań grupowych w klubie (np. "Najwięcej ton podrzuconych w grudniu"), każda zalogowana seria od razu aktualizuje ranking u pozostałych użytkowników. Oglądanie, jak punkty oponenta z drużyny rosną w czasie rzeczywistym przed naszymi oczami tworzy niezwykle wciągającą i motywacyjną grywalizację, niespotykaną w innych appkach.
10012. **Autorski System Choreografii Haptycznej i Dźwiękowej [Slavia-mobile]** - Flutterowy silnik sygnałów wibracyjnych. Zamienia klasyczne powiadomienia smartfona we wzorce haptyczne naśladujące sytuacje ze sceny ciężarowej. Posiada ostre uderzenia, gdy przypomina o starcie oraz cichsze "mruczenia" wspierające fazę skupienia przed podniesieniem ciężaru. Zamiast męczących dzwonków telefonicznych wykorzystuje dźwięki znane z pomostu sędziowskiego (buczki, dzwonki na zmianę ciężaru).
10013. **Automatyczny Montażysta „Highlight Reel” (Wideo) [Slavia-backend]** - Worker działający w tle wykorzystujący identyfikację momentów kluczowych i nasilenie głośności (np. krzyk uwalniający po udanym boju, potężny trzask rzuconej sztangi na pomost). Moduł wybiera najlepsze sekundy z surowego nagrania, docina klatki, nakłada brand Slavii i dodaje naklejkę z wyliczonym ciężarem, produkując gotowy, niesamowicie wyglądający 15-sekundowy „Short”/Rolskę do automatycznego wstawienia na media społecznościowe klubu.
10014. **GraphQL API Gateway [Slavia-backend]** - Ewolucyjne przearanżowanie klasycznych widoków RESTowych i utworzenie elastycznego grafu danych z wykorzystaniem biblioteki `async-graphql`. Klient (czy to Nuxt, czy urządzenie mobilne) pobiera idealnie skrojony payload, co całkowicie niweluje problemy "Over-fetchingu" (pobierania zbyt dużej liczby danych ze złączeń bazodanowych). Dla zawodnika ładującego ekran w słabym zasięgu to o połowę krótszy czas ładowania (TTI).
10015. **Wielo-Okienkowa Przestrzeń Dowodzenia (Split-Screen / Master-Detail) [Slavia-mobile]** - Pełne wykorzystanie możliwości "Adaptive Layout" we Flutterze z myślą o tabletach typu iPad. Zamiast ukrywać menu w szufladach, ekran jest dzielony horyzontalnie i wertykalnie. Z lewej strony trener obsługuje na stałe strumień analityczny wideo, pośrodku wypełnia tabelę dziennika, a z prawej na stałe zablokowany jest ekran kalkulatora Sinclaira, tworząc prawdziwy "dashboard terminalowy" dla wymagającego sztabu trenerskiego.
10016. **Edytor Planów Treningowych jako Graf Skierowany (DAG - Blueprints) [Slavia-frontend]** - Absolutna rewolucja w sposobie rozpisywania makrocykli. Zamiast wklepywać tekst w tabele, trenerzy dysponują narzędziem wizualnym z "węzłami" (podobnie jak systemy Node w narzędziach 3D czy programowaniu wizualnym). Rozdzielenie dni odbywa się po przeciągnięciu warunków: np. „JEŚLI odczucie bólu kolana w poniedziałek > 5 -> skieruj zawodnika na blok ćwiczeń B (izolacje)”. 
10017. **Analiza wektora balansu stopy przy użyciu Computer Vision [Slavia-mobile]** - Warstwa nakładająca AR (Augmented Reality) podczas nagrywania od boku. Aplikacja rysuje matematyczną linię czerwoną opadającą od środka gryfu. Jeśli linia w trakcie pociągnięcia czy siadu drastycznie przesunie się na tył pięty zawodnika lub palce, ekran błyska lekko i podświetla ten parametr na wyeksportowanym nagraniu, korygując jedną z największych plag u nowicjuszy dwuboju - utratę optymalnego środka ciężkości.
10018. **Mikroserwis Telemetrii i Metryk Biznesowych Klubu (Grafana Stack) [Slavia-backend]** - Odcięcie danych analitycznych (jak często poszczególne widoki są otwierane, w jakich porach zawodnicy najczęściej wpisują wyniki, na których ćwiczeniach użytkownicy najszybciej się "poddają") i przesyłanie ich anonimowo na dedykowany endpoint metryk Rusta. Możliwość generowania heatmap i metryk zaangażowania daje administracji klubu surowe dane na temat tego, jak kierować swoją energię i jak ulepszać platformę.
10019. **Moduł „Swipe-Readiness” (System Ankiet Ala Tinder) [Slavia-mobile]** - Zmiana formy zapytań kontrolnych z typowych szarych pól formularzy tekstowych na wielkie i błyskawiczne karty wizualne. Pytanie na karcie: "Jak jakość dzisiejszego snu?". Zawodnik przesuwa palcem szybko kartę w prawo (dobrze) lub w lewo (źle). Cała trójfazowa ankieta gotowości RPE przed przystąpieniem do ciężarów zostaje sprowadzona do formy, którą młodzież realizuje instynktownie i niemal z przyjemnością, zyskując 100% stopień wypełnialności.
10020. **Programowalne Reguły i Skrypty Notyfikacyjne IFTTT (If-This-Then-That) [Slavia-backend]** - Silnik "akcja-reakcja", w którym Zarząd Klubu może komponować zautomatyzowane lejki komunikacyjne i interwencyjne w systemie. Przykład skryptu: `Jeżeli (Zawodnik.Frekwencja_Tygodniowa < 1 ORAZ Zbliżają_się_zawody == TRUE) TO { Wyślij_Push_do_Zawodnika; Oflaguj_Na_Czerwono_w_Panelu_Trenera; }`. Pół-magiczne działanie zapobiegające zagubieniu poszczególnych atletów, całkowicie odciążające mózg trenera z ciągłego przeszukiwania tabel i list nieaktywnych sportowców.
10021. **Tryb „Kiosk-Mode” oraz Błyskawiczne Logowanie NFC [Slavia-frontend]** - Specjalny tryb zablokowany, w którym odpala się webowa aplikacja na terminalu wiszącym w hali treningowej. Posiada ultra-uproszczony "dotykowy" interfejs. Zawodnik wchodzący na salę przykłada do czytnika tabletu swój klubowy brelok (NFC) z zakodowanym UID lub szybko "wklepuje" 4-cyfrowy PIN, system loguje jego wejście (timestamp obecności), po czym od razu wyświetla wielkim fontem przypisany na dany dzień trening, samodzielnie czyszcząc sesję po odłączeniu palca.
10022. **Rozdzielony Architektonicznie Moduł Finansowy na Zgodności PCI-DSS [Slavia-backend]** - Hermetyczna obudowa logiki składek członkowskich od strony API. Wprowadzenie wirtualnych kont (np. via integracja Stripe Connect) zarządzanych poprzez bezstanowe weryfikacje Webhook. Oddzielenie bazy wrażliwych operacji mikropłatności od baz przechowujących zdjęcia z treningu, daje bezbłędną skalowalność płatności i umożliwia bezbolesne wprowadzanie np. opłat jednorazowych, karnetów partnerskich czy Klubowego Marketplace'u z odzieżą Slavia.
10023. **Generatywna Analiza Techniczna (LLM-Based) + Odtwarzacz Motywacyjny [Slavia-backend]** - Podłączenie wewnętrznego modelu analizy tekstu (np. lekkiego własnego modelu LLM vLLM). Narzędzie czyta serię surowych notatek, błędów i wskaźników pozostawionych przez trenera w danym tygodniu (np. "krzywy chwyt", "szybki drop") i formuje z tego płynne, motywacyjne "Listy Trenera" oraz zwięzłe podsumowania wygłaszane przez Text-To-Speech (lektor AI), co buduje niesamowitą, intymną interakcję z systemem.
10024. **Dynamiczne "Skeleton Loaders" i Progressive/Partial Hydration [Slavia-frontend]** - Ekstremalne wyżyłowanie wydajności renderowania Vue.js. Przy ładowaniu strony szkielety (skeletons) są dopasowane co do piksela do struktury JSON zwracanej z Rust API, co sprowadza metrykę CLS (Cumulative Layout Shift) idealnie do zera. Zastosowanie Progressive Hydration sprawia, że skrypty JS pobierają się tylko tam, gdzie użytkownik najeżdża kursorem, odciążając w ten sposób urządzenia z małymi zasobami RAMu.
10025. **Ochrona Algorytmiczna (Automatyczne Testy Mutacyjne) [Slavia-backend]** - Użycie bibliotek środowiska Rust (np. `cargo-mutants`) do ochrony absolutnie kluczowego kodu aplikacji (jak obliczenia składek, przeliczenia współczynnika Sinclair, przydział limitów wagowych w kategoriach). Ten rodzaj testów systematycznie, sam z siebie psuje kompilowany kod testowy (mutuje operatory) podczas Pipeline'u CI/CD, weryfikując, czy standardowe testy jednostkowe na pewno podniosą alarm. Zapewnia to stabilność matematyczną rodem z aplikacji bankowych.
10026. **Sterowanie Odtwarzaczem Wideo przy Pomocy Danych Żyroskopowych [Slavia-mobile]** - Nowatorskie podejście do mobilnego debugowania błędów boju w aplikacji. Podczas wyświetlania analizy wideo na pełnym ekranie w trybie zwolnionego tempa, zawodnik/trener po włączeniu specjalnego trybu może przesuwać palcem w lewo-prawo, lub obracać subtelnie nadgarstek trzymający telefon (odczyty z akcelerometru/żyroskopu). Ten fizyczny ruch nawija wideo do tyłu lub przewija do przodu, ułatwiając pauzowanie z chirurgiczną precyzją w ułamku sekundy nad gryfem.
10027. **Scentralizowany System „Design Tokens” [Slavia-frontend/Slavia-mobile]** - Zmiana infrastruktury wizualnej. Wszystkie kolory (czerwienie Slavii, odcienie bieli), typografie (skale fontów) oraz wartości marginesów zostają wyprowadzone do jednego potężnego pliku JSON repozytorium nadrzędnego. Na jego podstawie potok deweloperski "wypluwa" zmienne do `.css` w Nuxcie i klasy `ThemeData` dla Darta we Flutterze, zapewniając niesamowitą spójność ekosystemów i błyskawiczne "White-Labeling" dla potencjalnie chętnych klubów partnerskich.
10028. **Interaktywny Menedżer Zbijania Wagi (Weight Cut Planner) [Slavia-mobile]** - Rezygnacja z zewnętrznych kalkulatorów makrosów. Narzędzie spięte wprost z oficjalnymi kategoriami IWF i z osią czasu na platformie. Analizuje trend spadku masy ciała ze zgłaszanych wpisów, weryfikując go algorytmem ułatwiającym śledzenie deficytu kalorycznego i stopnia dehydratacji (nawodnienia) przed startami. Zapewnia wizualizacje przypominające, w jakim zakresie kilogramów bezpiecznie operować, minimalizując ubytki mocy na pomostach.
10029. **"Silnik Epidemiologiczny" Kontuzji Pomiędzy Zawodnikami [Slavia-backend]** - Agregacyjny model badający anonimizowane, tagowane zgłoszenia bólów w obrębie danego miesiąca z dzienników klubu. Jeśli narzędzie detekuje, że po drastycznym skoku objętości siadów bułgarskich nagle 30% grupy odczuwa skoki bolesności więzadeł rzepki – generowany jest alert w czerwonym kolorze na tablicy administracyjnej trenerów, pozwalając na szybką autokorektę makrocyklu w skali całej sekcji klubu.
10030. **Moduł Inteligentnej Rezerwacji Pomostów (Platform Booking System) [Slavia-frontend]** - Wizualna symulacja floor-planu (rzut siłowni z góry) dostępna z telefonu. Przy zatłoczonych godzinach popołudniowych w Slavii, zawodnicy rezerwują stanowisko i slot z góry, gwarantując sobie czas na platformie lub w klatkach. Wszelkie rezerwacje synchronizują się z harmonogramem trenerów (którzy mogą zorganizować mini-sesję z konkretną grupą 3 zawodników, blokując im wybrany obszar). System zmniejsza chaos operacyjny i eliminuje problem sprzętowy w małych obiektach.

---

## Bank pomysłów 10031-10050 (Szlifowanie i Ulepszanie Istniejących Funkcji)

~~10031. **Refaktoryzacja UI Ekranu Logowania [Slavia-mobile]** - Zmiana dotychczasowego prostego formularza na nowoczesny ekran z subtelną animacją rozmytego tła (glassmorphism) oraz wideo trenującego sztangisty w tle. Zmiana działania pól tekstowych: dynamiczne etykiety (floating labels) oraz widoczny z poziomu klawiatury przycisk "Dalej/Zaloguj" bez konieczności zjeżdżania w dół ekranu.~~
10032. **Płynniejsze Przejścia w Kalendarzu [Slavia-frontend]** - Zamiast nagłego przeładowywania danych przy zmianie miesiąca na widoku kalendarza, wdrożenie płynnej animacji "Slide" (przesunięcia karuzeli). Dodatkowo, kolorowanie dni z zaplanowanym treningiem na widoku miesięcznym powinno używać gradientowych kropek odzwierciedlających intensywność (RPE/Tonaż) przypisanego planu.
10033. **Dynamiczny UX Kalkulatora Sinclair [Slavia-frontend]** - Obecny kalkulator wymaga ręcznego wpisywania wagi. Ulepszenie: po wejściu zalogowanego użytkownika suwaki są już pre-definiowane na podstawie jego ostatniej wagi z profilu, a wynik renderuje się dynamicznie (animowany licznik rosnący w czasie rzeczywistym) przy każdym pociągnięciu suwaka masy sztangi.
10034. **Inteligentne Grupowanie Powiadomień [Slavia-mobile]** - Obecna lista powiadomień jest po prostu strumieniem. Usprawnienie polega na pogrupowaniu ich w sekcje "Dzisiaj", "Wczoraj" ze wspólnym awatarem dla seryjnych powiadomień z tego samego źródła (np. 3 przypomnienia od trenera zwijają się do jednego kafelka typu "Rozwiń wątek").
10035. **Wizualna Karta Ćwiczenia w Planach [Slavia-frontend]** - W widoku przypisywania planu zmiana suchych list tekstowych (np. "Rwanie 3x3") na bogate wizualnie "Karty" z miniaturką ćwiczenia/ikony mięśni i chipami (np. tagi: "Siła", "Technika"). Trenerzy będą mogli przeciągać te karty (drag-and-drop) przy układaniu bloku treningowego, co jest 10x szybsze niż wybór z rozwijanej listy.
10036. **Skrócenie Ścieżki Zgłaszania Ćwiczeń [Slavia-frontend/Slavia-backend]** - Obecnie zawodnik musi wejść w edycję wpisu, aby podać wykonane powtórzenia. Usprawnienie: dodanie metody PATCH "Quick Complete". Na froncie zawodnik widzi przycisk "Zalicz jako zaplanowane" i jednym kliknięciem zatwierdza całą serię bez otwierania modala edycji.
~~10037. **Optymalizacja Cache'owania Wyników [Slavia-backend]** - Na endpointach z wynikami wdrożenie tagów `ETag` (Entity Tag) i warunkowych żądań GET (`If-None-Match`). Dzięki temu aplikacja mobilna przestanie w kółko pobierać listę wyników zawodnika, jeśli nic się nie zmieniło na serwerze, redukując zużycie bazy i zapytań o ponad 80% przy przełączaniu zakładek.~~
10038. **Zmiana Kontrolki Wyboru Zawodnika z Command Palette [Slavia-frontend]** - Przypisywanie planu konkretnemu zawodnikowi bywa uciążliwe przy dużym klubie. Zastąpienie standardowego paska wyboru pełnoprawnym komponentem "Command Palette" – po naciśnięciu skrótu (np. Ctrl+K) trener szybko wpisuje imię, system autouzupełnia, a Enter natychmiast przypisuje zawodnika.
10039. **Poprawa Estetyki i Czytelności Changeloga [Slavia-frontend]** - Widok `/admin/changelog` wyświetla suche informacje z Markdowna. Przepuszczenie danych przez komponent "Timeline", który automatycznie dodaje kolorowe tagi (np. niebieski dla [Frontend], rdzawy dla [Rust]) i wyświetla ikonki zmian, sprawiając, że historia wersji jest krystalicznie czytelna.
10040. **Dynamiczny App Bar i Środowisko Czytania [Slavia-mobile]** - W ekranach podrzędnych zmiana sztywnej górnej belki (AppBar) na wariant zwijany ("SliverAppBar"). Kiedy zawodnik przewija swój dziennik w dół, belka kurczy się lub znika, robiąc miejsce dla treści, a z tyłu pojawia się subtelne rozmycie (blur) elementów, które pod nią znikają.
10041. **Animacje Przejść Między Ekranami [Slavia-frontend]** - W Nuxt.js skonfigurowanie natywnych, łagodnych przejść (`page-transition`). Zamiast błyskawicznego podmieniania komponentów przy zmianie podstron (np. z Profilu na Kalendarz), zastosowanie delikatnego efektu "Crossfade" lub "Slide", co nada wrażenie nowoczesnej, płynnej aplikacji natywnej.
10042. **Zwiększenie Wyrazistości Dziennika Audytu [Slavia-frontend]** - Na liście logów superadmina każdy wpis zlewa się w całość. Zmiana: logi destrukcyjne (DELETE, modyfikacje uprawnień) otrzymują pogrubione, czerwone oznaczenia, a operacje udane (LOGIN, CREATE) są subtelniejsze. Najechanie kursorem ujawnia zagnieżdżony kod JSON w estetycznie sformatowanym boksie (Syntax Highlighting).
10043. **Migoczące Szkielety Ładowania w Mobilce [Slavia-mobile]** - Przed załadowaniem list z zewnętrznego API, zawodnik widzi przez chwilę puste tło. Dodanie migoczących, szarych atrap elementów (Shimmer Loading) odpowiadających kształtem wpisom treningowym całkowicie wyeliminuje odczucie "zacięcia" aplikacji podczas "zimnego" startu i synchronizacji.
10044. **Zmiana Logiki Odnawiania Sesji (Sliding Window) [Slavia-backend]** - Ulepszenie mechanizmu wygasania JWT. Zamiast brutalnie wyrzucać trenera na ekran logowania, jeśli token ulegnie wygaśnięciu, zaimplementowanie "Sliding Window" na backendzie – jeśli użytkownik jest aktywnie przeklikujący system, autoryzacja odnawia token "od spodu" na kolejne godziny.
10045. **Interaktywne Wykresy Progresu [Slavia-frontend]** - Aktualizacja statycznych linii wykresów w profilu zawodnika. Przy dotknięciu na "kropkę" konkretnego boju (np. Rwanie 100 kg), pojawi się tooltip z wynikiem oraz bezpośrednim przyciskiem/linkiem kierującym od razu do dnia w kalendarzu lub zapisanego wideo z tego konkretnego treningu.
10046. **Szybki Przełącznik Motywów w Belce Globalnej [Slavia-frontend]** - Rozszerzenie ikonki profilu w prawej górnej belce o błyskawiczny przełącznik (switch) Trybu Ciemnego / Jasnego z gładką animacją maski (rozchodzącego się promienia na ekranie). Motyw natychmiastowo odpowiada, a zapis dokonuje się do `localStorage` w tle.
10047. **Leniwe Ładowanie Osadzonych Mediów na Osi Czasu [Slavia-frontend]** - Na długiej osi czasu, ładowanie wszystkich zdjęć na raz spowalnia przeglądarkę i zużywa pakiety mobilne. Wdrożenie natywnego leniwego ładowania (`loading="lazy"` + observer dla komponentów w Vue), dzięki któremu klatki pobierają się dopiero przed wejściem w strefę okna (Viewport).
10048. **Reaktywne Blokady Przed Utratą Formularza [Slavia-frontend]** - Dodanie strażnika nawigacji ("Dirty State Checking") na widoku redagowania dziennika. Jeśli zawodnik lub trener zaczął wpisywać powtórzenia i przez przypadek uderzył przycisk Wstecz, system natychmiastowo zatrzymuje go wbudowanym powiadomieniem: "Masz niezapisane zmiany. Porzucić formularz?".
10049. **Zagęszczony Tryb Pracy dla Trenerów [Slavia-frontend]** - Na mniejszych laptopach karty zawodników zabierają zbyt dużo przestrzeni w pionie. Dodanie małej ikony podziału w rogu, przełączającej wizualizację siatki (Karty) na gęsty układ tabelaryczny. Kompaktowy widok zmieści statusy, wagi i gotowość nawet 15-20 osób na jednym widoku ekranu.
10050. **Sprytniejsze Filtrowanie Endopintów za Pomocą Query Bindera [Slavia-backend]** - Usprawnienie architektury w Axum, tak aby wszystkie skomplikowane operacje "szukaj po dacie, minimum punktach, płci" obsługiwać potężnym mechanizmem wstrzykiwanym do zapytań SQL bezpośrednio z URL. Wyręczy to frontend z filtrowania masywnych zestawień po stronie klienta, dając drastyczne przyspieszenie wczytywania list.

---

## Bank pomysłów 10051-10150 (Massive Polish & QoL Improvements)

10051. **Animacje Przycisków Akcji [Slavia-mobile]** - Dodanie subtelnego efektu "wklęśnięcia" (scale down) przy naciskaniu każdego głównego przycisku w aplikacji, zamiast natychmiastowej zmiany koloru.
10052. **Auto-fokus na polu "Waga" [Slavia-frontend]** - W modalu edycji dziennika natychmiastowe ustawienie kursora w polu wprowadzania ciężaru po jego otwarciu, redukujące konieczność klikania myszką.
10053. **Kolorowanie Składni w Błędach Walidacji [Slavia-backend]** - Ujednolicenie odpowiedzi 422 Unprocessable Entity, tak aby komunikaty błędów zawierały klucz pola w osobnej obwolucie (np. `[waga] Wartość nie może być ujemna`).
10054. **Pull-to-Refresh w Dzienniku [Slavia-mobile]** - Zmiana przycisku odświeżania na natywny gest przeciągnięcia w dół ekranu na liście treningów i osi czasu.
10055. **Sticky Headers w Tabelach [Slavia-frontend]** - Zablokowanie górnego wiersza (nagłówka) we wszystkich dużych tabelach w panelu administracyjnym podczas przewijania wyników w dół.
~~10056. **Kompilacja AOT dla Fluttera [Slavia-mobile]** - Optymalizacja skryptów budujących CI/CD pod kątem restrykcyjnego Ahead-Of-Time compilation dla profilu wydania, co zmniejszy rozmiar aplikacji o ~15%.~~
10057. **Wizualizacja "Pustego Stanu" (Empty States) [Slavia-frontend]** - Zamiana suchych tekstów "Brak danych" na estetyczne grafiki wektorowe z motywacyjnym wezwaniem do akcji (np. ikonka sztangi + "Zgłoś swój pierwszy wynik!").
10058. **Odchudzenie Payloadu Profilu [Slavia-backend]** - Usunięcie z odpowiedzi `/api/athletes/me` potężnych pól bazy (np. avatary w Base64, jeśli tam są) i zastąpienie ich odnośnikami do presigned URL w celu szybszego ładowania autoryzacji.
10059. **Wibracje Haptyczne przy Rejestracji [Slavia-mobile]** - Lekkie "tyknięcie" przy wpisywaniu pinu logowania w aplikacji lub weryfikacji biometrycznej.
10060. **Tooltip "Ile dni temu" [Slavia-frontend]** - Zmiana suchych dat w osi czasu (np. "2026-05-18") na format "2 dni temu (18 maja)" przy użyciu biblioteki dayjs.
10061. **Kompaktowe Karty Zawodów [Slavia-mobile]** - Zmniejszenie wysokości kart w `/screens/calendar_screen.dart`, aby na małym ekranie mieściły się 4 zawody jednocześnie zamiast 2.
10062. **Ujednolicone Marginesy Typografii [Slavia-frontend]** - Przepisanie `app.css` aby wszystkie nagłówki od H1 do H4 posiadały proporcjonalne wcięcia wynikające z ustandaryzowanej skali modularnej (np. 1.25x).
10063. **Cache'owanie Zapytań Axios/Fetch [Slavia-frontend]** - Wpięcie interceptora, który przez 5 sekund trzyma w pamięci odpowiedzi GET dla statycznych słowników (np. lista kategorii wagowych), aby nie uderzać do API przy każdym montowaniu komponentu.
10064. **Skrypty Migracyjne w Axum [Slavia-backend]** - Przebudowa logiki startowej serwera na automatyczne sprawdzanie i aplikowanie brakujących migracji SQLite (`sqlx migrate run`) od razu przy restarcie aplikacji.
10065. **Wskaźnik Bezpieczeństwa Hasła [Slavia-frontend]** - Zmiana suchego komunikatu o długości hasła na zielono-czerwony pasek siły zabezpieczenia (z animacją przejścia) w panelu ustawień 2FA.
10066. **Grupowanie Osi Czasu po Miesiącach [Slavia-mobile]** - Podzielenie długiej listy wpisów na bloki oddzielone wyraźnym nagłówkiem miesiąca (np. "Kwiecień 2026"), co drastycznie ułatwi przewijanie.
10067. **Ograniczenie Rate-Limit dla Zapomnianego Hasła [Slavia-backend]** - Ustawienie surowego limitu w oparciu o Redis/memory na endpoint restartu hasła, aby uniemożliwić masowe wysyłanie spamu e-mail do użytkowników.
10068. **Debounce Wyszukiwarki Globalnej [Slavia-frontend]** - Ustawienie 300ms opóźnienia (`useDebounceFn`) na pasku wyszukiwania w panelu trenera, by przestać zasypywać serwer nowym żądaniem przy wpisaniu każdej pojedynczej litery.
10069. **Zwijanie Notatek Trenera [Slavia-mobile]** - Jeśli komentarz trenera przekracza 3 linijki, ukrycie go za estetycznym przyciskiem "Czytaj dalej...", by nie psuć widoku osi czasu.
10070. **Formatowanie Pieniędzy (Currency) [Slavia-frontend]** - Zastosowanie `Intl.NumberFormat` na wszystkich widokach płatności (`/superadmin` i `/athlete/skladki`), by automatycznie formatować kwoty np. `150.00 PLN` zamiast sztywnego tekstu.
10071. **Ulepszone Okno Dialogowe Potwierdzeń [Slavia-frontend]** - Zamiast natywnego szpetnego okienka `window.confirm()`, wdrożenie globalnego modułu modala Nuxt UI dla operacji np. usunięcia planu treningowego.
10072. **Lokalne Logowanie Błędów HTTP [Slavia-backend]** - Dodanie warstwy `tower_http::trace` do Axum, aby każde zjawisko kodu HTTP > 400 pozostawiało czysty, uporządkowany log w konsoli z adresem IP i czasem trwania żądania.
10073. **Rozmycie Kart Odrzuconych [Slavia-mobile]** - Odrzucone (zfailowane) wpisy w dzienniku powinny mieć obniżone krycie (opacity: 0.6), aby automatycznie na pierwszy rzut oka wyróżniały się pozytywne, zielone rekordy.
10074. **Przejścia Hover na Ikony [Slavia-frontend]** - Dodanie w klasach Tailwind efektu `transition-colors duration-200` na każdym guziku narzędziowym (np. edycja ołówkiem, kosz) dla lepszego uczucia "ciężkości" interfejsu.
10075. **Przyjazne URL (Slugification) [Slavia-backend]** - Aktualizacja schematu bazodanowego dla aktualności klubowych, aby odnośniki opierały się na tytule (np. `/news/wygrana-slavii`) zamiast nieczytelnych identyfikatorów UUID.
10076. **Aktywne Stany Menu Bocznego [Slavia-frontend]** - Podświetlenie (bold i tło) dokładnie tego linku w Sidebarze, na którym znajduje się trener (nawet na głęboko zagnieżdżonych trasach typu `/trainer/cwiczenia/id`).
10077. **Optymalizacja Nawigacji Dolnej (Bottom Nav) [Slavia-mobile]** - Wyłączenie przeładowywania korzenia aplikacji po kliknięciu ikony w BottomNavigationBar we Flutterze; zastosowanie IndexedStack dla utrzymywania stanu zakładek.
10078. **Bezpieczny Render Markdown [Slavia-frontend]** - Wymuszenie rygorystycznej konfiguracji biblioteki do renderowania changeloga (`admin/changelog.vue`), zapobiegającej ewentualnym atakom XSS przy wyciąganiu opisów łatek.
10079. **Szybsza Kompresja Zdjec [Slavia-backend]** - Podmiana silnika zmiany rozmiaru awatarów w Rust na ultra-szybkie bliblioteki np. `fast_image_resize` operujące w całości na pamięci podręcznej SIMD.
~~10080. **Scroll-to-Top Button [Slavia-frontend]** - Pływający w prawym dolnym rogu przycisk strzałki pojawiający się tylko po zjechaniu na ekranie o 500px, natychmiastowo wracający użytkownika na górę długich rankingów.~~ *(wdrożone: `SlaviaScrollToTop` w `PublicPageLayout` — strony publiczne z długą treścią.)*
10081. **Podświetlenie Edytowanych Pól [Slavia-mobile]** - Podczas modyfikacji dziennika pole, które zostało zmienione względem stanu z API powinno zyskać żółtą obwódkę sygnalizującą status "Dirty".
10082. **Preload Czcionek [Slavia-frontend]** - Deklaracja kluczowych rodzin fontów z flagą `rel="preload"` w tagu `<head>` strony, aby całkowicie wyeliminować miganie tekstu (FOIT) przy pierwszym ładowaniu.
10083. **Wyrównanie Kolumn w Tabelach [Slavia-frontend]** - Ręczne przypisanie atrybutów np. `text-right` dla kolumn zawierających wagi (kg) i wartości procentowe w wynikach, aby łatwiej czytało się je w pionie.
10084. **Optymalizacja Obiektów DTO [Slavia-backend]** - Rezygnacja ze zwracania całych struktur bazodanowych z Axum; stworzenie mniejszych, ściśle zdefiniowanych structów z tagami `#[serde(skip_serializing_if = "Option::is_none")]`.
10085. **Natywne Okno Udostępniania [Slavia-mobile]** - Użycie pakietu `share_plus` aby po kliknięciu na certyfikat Sinclaira uruchomić systemowy iOS/Android Share Sheet zamiast ułomnego kopiowania wewnątrz apki.
10086. **Blokada Podwójnych Zgłoszeń Formularzy [Slavia-frontend]** - Nadanie stanu `:disabled` na wszystkie przyciski "Zapisz" z chwilą zainicjowania żądania fetch, aż do momentu odpowiedzi serwera (zapobiega spamowaniu bazy przez nerwowe klikanie).
10087. **Oznaczenia Czasu Względem Stref [Slavia-backend]** - Ujednolicenie logiki Rusta do zwracania i zapisywania wszystkich dat bezwzględnie w UTC (w formacie ISO 8601), a parsowanie na `Europe/Warsaw` odbywać się ma wyłącznie na urządzeniu klienta.
10088. **Focus Trapping w Modalach [Slavia-frontend]** - Narzucenie dostępności (A11Y) w taki sposób, aby nawigacja klawiszem TAB nie opuszczała otwartego modala trenera na tło strony.
10089. **Czyste Puste Listy Startowe [Slavia-mobile]** - Gdy zawodnik nie ma na dany dzień treningu, kalendarz we Flutterze nie powinien pokazywać czarnej pustki, lecz motywujący cytat o regeneracji (np. z losowej puli).
10090. **Autouzupełnianie Hantli/Ciężaru [Slavia-frontend]** - Pole input "ciężar" w planowaniu treningu powinno domyślnie posiadać typ "number" ze stepem 2.5kg (odpowiadającym najmniejszym talerzom na hali).
10091. **Agregacja Logów do Pliku [Slavia-backend]** - Aktualizacja konfiguracji `tracing_appender`, aby archiwizowała logi dzienne (rolling file appender) w formacie JSONL zamiast wyrzucać wszystko tylko na stdout terminala.
10092. **Zmiana Maskowania Haseł [Slavia-mobile]** - Ikona przekreślonego oka w polach z hasłem, pozwalająca użytkownikowi szybko przejrzeć wprowadzony kod (istotne na małych klawiaturach).
10093. **Ikony Typów Ćwiczeń [Slavia-frontend]** - Przyznanie małych znaków SVG obok nazw (np. znak płomienia przy siadach, znak błyskawicy przy ciągach rwaniowych), dla łatwiejszego skanowania wzrokiem harmonogramu.
10094. **Skrypty Graceful Shutdown [Slavia-backend]** - Dodanie obsługi sygnałów SIGINT/SIGTERM w main.rs zamykających pulę połączeń bazodanowych (SqlitePool) w kontrolowany sposób przy aktualizowaniu serwera.
10095. **Kolorowe Kategorie Wagowe [Slavia-frontend]** - Przypisanie sztywnego zakresu kolorystycznego (badge) z palety Tailwind dla najpopularniejszych kategorii wiekowych U17, U20, Senior, ułatwiających czytanie list startowych adminom.
10096. **Autozapis Formularzy Redakcyjnych [Slavia-mobile]** - Implementacja małego timera co 10 sekund zrzucającego aktualny wpis do `SharedPreferences` we Flutterze. Jeśli apka ulegnie crashowi, nie utracimy danych.
10097. **Walidacja Unikalnego Adresu E-mail [Slavia-backend]** - Odesłanie wyraźniejszego kodu HTTP `409 Conflict` (zamiast generycznego 400) ze wskazaniem dla Frontendu, że konto na podany adres już widnieje w systemie.
10098. **Wyróżnienie Dzisiejszej Daty [Slavia-frontend]** - W gridzie `/kalendarz` komórka odpowiadająca dniu dzisiejszemu musi być oznaczona mocną czerwoną ramką lub pełnym tłem, aby natychmiast budowała orientację w czasie.
10099. **Zwijana Belka Aktualności [Slavia-mobile]** - Ograniczenie tekstów aktualności na liście do maksymalnie 3 linijek z parametrem `TextOverflow.ellipsis`, by długie posty klubu nie "rozpychały" interfejsu.
10100. **Pasek Zmian Procentowych w Sinclair [Slavia-frontend]** - Widget przy profilu Sinclaira pokazujący obok liczby różnicę z poprzedniego miesiąca w formacie "↑ 2.4%" na zielono lub na czerwono.
10101. **Ograniczenie Rozmiaru Puli Zapytań Sqlite [Slavia-backend]** - Dokonfigurowanie w `sqlx` maksymalnej ilości połączeń (max_connections) dostosowanej do limitów zasobów wirtualnej maszyny zamykając błędy `Too many open files`.
10102. **Dynamiczny Placeholder Avataru [Slavia-frontend]** - Generowanie inicjałów (np. "JK" dla Jan Kowalski) na szarym tle, jeśli użytkownik nie dodał jeszcze własnego zdjęcia w module ustawień konta.
10103. **Animacja Zmiany Oceny Gotowości [Slavia-mobile]** - Przy przesuwaniu suwaka RPE od 1 do 10 ikona twarzy "usmieszka/zmęczenia" płynnie morphuje się w odpowiednią emocję w dzienniku wprowadzania wyników.
10104. **Prekompilowane Szablony Maili [Slavia-backend]** - Wyrzucenie surowego kodu HTML generowanego z logiki na rzecz silnika szablonów np. `askama` (kompilowanego przy buildzie Rusta).
10105. **Breadcrumbs w Modułach Trenera [Slavia-frontend]** - Dodanie paska "Okruszków" na samej górze (np. `Klub / Plany / Tydzień A / Rwanie`), by trener nie gubił się w hierarchii rozbudowanego układu dniowego.
10106. **Grupowanie Opcji Wyboru Dat [Slavia-mobile]** - Zamiana kółek dat/czasu (Cupertino/Material picker) na uproszczony wybór kafelkowy "Dzisiaj", "Wczoraj", "2 Dni temu" używany w 90% sytuacji edycji logu.
10107. **Eksport JSON dla Audytów [Slavia-frontend]** - Rozszerzenie widoku `superadmin/audit-logs.vue` o przycisk szybkiego pobierania logów do niesformatowanego pliku json/csv dla księgowych.
10108. **Kompresja Brotli w Middleware [Slavia-backend]** - Zastąpienie standardowego gzipa poprzez dopięcie `tower_http::compression` z wyższym poziomem Brotli kompresującym payloady tekstowe Nuxta o dodatkowe 20%.
10109. **Walidacja Mocy Treningu w Kółku [Slavia-frontend]** - Jeśli łączny tonaż dnia przekracza drastycznie standardy danego zawodnika, progress-bar zmienia kolor na pomarańczowy, ostrzegając przed ułożeniem "nierealistycznego planu".
10110. **Taptic Engine przy Dodawaniu Serii [Slavia-mobile]** - Gdy zawodnik podczas treningu naciska "+" przy dodaniu kolejnej zrobionej serii powtórzeń na siłowni, urządzenie odpowiada lekką, szybką sprężystą wibracją.
10111. **Podział Widoku na Role w SuperAdminie [Slavia-frontend]** - W widoku uprawnień dodanie jasnego przełącznika "Role" (Trener / Admin / Zawodnik) filtrującego wielką listę kont w firmie.
10112. **Blokada Wymaganych Pól w SQLite [Slavia-backend]** - Uporządkowanie definicji schematu, wymuszenie w constraintach `NOT NULL` tam gdzie Axum i tak weryfikuje istnienie struktury (np. ID przypisanego atlety).
10113. **Dynamiczna Wysokość Pola Tekstowego [Slavia-mobile]** - `TextField` na uwagi od trenera sam rozszerza się w pionie przy każdym wciśnięciu Enter, zamiast zmuszać do scrollowania maleńkiego wiersza.
10114. **Hover States na Opcjach Logowania [Slavia-frontend]** - W `/logowanie` najechanie na guziki zapomnianego hasła uaktywnia płynne podkreślenie tekstu (underline animation).
10115. **Kaskadowe Usuwanie Zasobów [Slavia-backend]** - Dopilnowanie reguł `ON DELETE CASCADE` w tabelach bazy – usunięcie zawodnika musi fizycznie czyścić jego pliki, plany, wpisy i rekordy z bazy chroniąc przed nagromadzeniem tzw. "Orphan Records".
10116. **Ostrzeżenie przed Zamknięciem Dialogów [Slavia-mobile]** - Gdy trener ma otwarty pełnoekranowy modal ułożonego tygodnia planu, dotknięcie poza tło nie powinno od razu odrzucać wykonanej pracy (system pyta o pewność).
10117. **Skróty Statusów Płatności [Slavia-frontend]** - Moduł składek otrzymuje czytelne kafelki podsumowujące (np. zielone tło = Zapłacono, czerwone = Zaległość powyżej 7 dni), natychmiast widoczne na wierszu zawodnika w siatce.
10118. **Zmiana Formatu Hashowania Haseł [Slavia-backend]** - Przejście z biblioteki z bcrypt na dużo mocniejszy wariant `Argon2id` z automatycznym upgradowaniem hasha po kolejnym pomyślnym zalogowaniu użykownika.
10119. **Krótkie Komunikaty Utraty Sieci [Slavia-mobile]** - Mały czerwony pasek "Brak połączenia" na samym dole (Snackbar) powiadamiający zawodnika o odłączeniu bez agresywnego wyskakiwania na cały ekran i blokowania akcji dziennika.
10120. **Przełącznik Skali (Kg / Lbs) w Formularzu [Slavia-frontend]** - Jeden uniwersalny `switch` zapisywany w preferencjach konta. Wewnątrz klubu trenują być może goście z zagranicy – formularz przeliczy wszystko inteligentnie w locie.
10121. **Animacja Spinnera przy Rejestracji [Slavia-mobile]** - Domyślne wbudowane kółko Fluttera (CircularProgressIndicator) zastąpione mniejszym, gładkim wektorem podnoszonej sztangi animowanym w trakcie komunikacji z API.
10122. **Logika Czystych Ścieżek Plików [Slavia-backend]** - Standardyzacja funkcji tworzenia nazw uploadowanych zdjęć lub nagrań w oparciu o maski `[uuid]-[rok]-[id_zawodnika].jpg` zapobiegające dublowaniu się kluczy przy ekstrakcji z bazy.
10123. **Skrolowanie Osi X na Tabelach [Slavia-frontend]** - Na urządzeniach mniejszych niż desktop (np. iPad), duże tabele wyników z 15 kolumnami ukrywają resztę i pokazują ładną belkę "Przesuń w prawo po więcej..." bez gniecenia układu strony.
10124. **Szybsza Aktualizacja Suwaków Kalkulatora Sinclaira [Slavia-mobile]** - Położenie sliderów we Flutterze ujęte w odrębny zoptymalizowany Widget bez przerysowywania reszty ogromnego ekranu za każdym przeciągnięciem rączki.
10125. **Tryb "Przeglądu" w Dzienniku Trenera [Slavia-frontend]** - Nowy checkbox (Tylko nieodczytane wpisy), dzięki któremu prowadzący dostaje zaledwie odfiltrowane dni swoich zawaodników oczekujące na zostawienie komentarza zwrotnego.
10126. **Odporność na Polskie Znaki i Spacje [Slavia-backend]** - Dodatkowa rygorystyczna sterylizacja i funkcja `trim()` w Rustcie przed jakimikolwiek weryfikacjami danych wejściowych z paska "Nazwa Klubu" czy "Imię Zawodnika".
10127. **Tooltip Kopiowania do Schowka [Slavia-frontend]** - Skopiowanie linku do profilu zawodnika z panelu admina od razu skutkuje eleganckim odciskiem małego napisu "Skopiowano!" przy kursorze myszki.
10128. **Wyróżnienie Dnia Startu w Kalendarzu [Slavia-mobile]** - Kropka treningowa na kalendarzu zyskuje u dołu ikonę pucharu zamiast standardowej białej łatki, by optycznie wzmocnić rangę wpisu dnia konkursowego z `competition.dart`.
10129. **Obcięcie Częstotliwości Autosave'u [Slavia-frontend]** - Moduł budowania planów "TrainingPlanBuilder" optymalizuje autosave: zapisze postęp dopiero po "wyjściu z inputa", a nie natychmiast przy klikaniu pojedynczych klawiszy by odciążyć bazę danych.
10130. **Rozdzielenie Transakcji na Wiele Dni [Slavia-backend]** - Procedura PATCH dla `cwiczenia.vue` (zapis planów na 5 dni do przodu) zawinięta w potężną pojedynczą transakcję `BEGIN/COMMIT` w SQLite gwarantująca absolutny brak częściowo "niedokończonych" tygodni przy awarii połączenia.
10131. **Skalowanie Layoutu Pod Niestandardowe Rozmiary Czcionek [Slavia-mobile]** - Przepisanie widgetów kalkulatora i dziennika aby reagowały przy uzyciu `FittedBox`, chroniąc interfejs przed katastrofą gdy zawodnik używa systemowego "Powiększ tekst x1.5".
10132. **Automatyczne Czyszczenie Filtrów Tabel [Slavia-frontend]** - Przycisk "Wyczyść" obok lupki, jednym naciśnięciem kasujący 6 różnych włączonych opcji, bez konieczności ponownego przewijania i odhaczania pojedynczych kratek na `/superadmin/index.vue`.
10133. **Ukrywanie Danych Wrażliwych po Braku Aktywności [Slavia-mobile]** - Po 5 minutach bez dotknięcia interfejsu, zawartość okien takich jak rachunki i skartki członkowskie zostaje rozmyta blur'em wymagając ponownego dotknięcia dla ujawnienia.
10134. **Konfiguracja "CORS" [Slavia-backend]** - Utwardzenie zabezpieczeń `corsLayer` przez twarde ograniczenie Origins na ścisłe wartości domen produkcyjnych frontendu (`slavia.pl` itd), zamykające drogę przed odpytywaniem z lokalnych serwerów hakerów.
10135. **Formatowanie Numeryczne Podpowiedzi [Slavia-frontend]** - Pola procentowe w planowaniu dają natychmiast z boku szarą przypominajkę ile to fizycznie na gryfie "80% = (80kg) z poprzedniego PR".
10136. **Łatwiejsza Obsługa Dni Wolnych [Slavia-mobile]** - Ekran modala do deklaracji niedostępności zawodnika, gdzie w kalendarzu szybko palcem można przeciągnąć przez cały tydzień by od razu ustawić mu zwolnienie L4 dla trenera.
10137. **Skrót na Pulpicie do Wpisania Wyniku (App Shortcuts) [Slavia-mobile]** - Długie wciśnięcie ikony aplikacji Slavii wyzwala menu skrótów "Dodaj nowy wynik" kierujące użytkownika od razu do paska wejściowego formularza z pominięciem ładowania ekranu głównego.
10138. **Czystość Kodu Pliku konfiguracyjnego `lib.rs` [Slavia-backend]** - Rozdzielenie głównego bloku na przejrzyste sub-moduły, z dedykowanymi sekcjami dla wtryskiwania zaleźności (Dependency Injection) ułatwiającemu nawigację po serwerze.
10139. **Szkieletowanie Wczytywania Planu [Slavia-frontend]** - Ukończenie loadera do `cwiczenia.vue`: układ kolumn i rubryk dla rwań pozostaje "wystrugany" szarą animacją zanim Nuxt doczyta do nich detale powtórzeń z serwera Axum.
10140. **Oszczędne Zapytania Uwierzytelniania w Composables [Slavia-frontend]** - Optymalizacja `useAuth.ts` na zrzucanie wyników dekodowanego obiektu stanu do globalnego Store (np. Pinia), drastycznie wyciszająca redundancję wapytań backendu przy rze-renderowaniu górnego menu.
10141. **Powiadomienia Typu "Ostrzeżenie" w Axum [Slavia-backend]** - Umożliwienie zwracania statusu `200 OK` ale z doczepionym do JSON polem `warning: "Konto wygasa za 2 dni"`, by interfejs frontendu mógł płynnie zasugerować trenerowi przedłużenie opłacania licencji.
10142. **Spłaszczone Listy Wymagań Hasła [Slavia-frontend]** - Odświeżenie procedury logowania: Wymagania długości i znaków specjalnych podświetlają się dynamicznie na zielono już w takcie uderzania w klawiaturę bez przesyłania enter'em.
10143. **Kolorowanie Przeterminowanych Kart Zawodów [Slavia-mobile]** - Przestarzałe zdarzenia startowe automatycznie ulegają obniżeniu krycia do koloru czarno-białego lub wyszarzają się uwalniając optycznie pole na rzecz nowo nadchodzących dat.
10144. **Szybszy Parsowanie Modeli Plikowych Datetime [Slavia-backend]** - Wymiana metodyk w `mobile_releases.rs` i reszcie projektów na optymalizowane biblioteki weryfikacji i operacji kalendarzowych dla zlikwidowania wąskich gardeł przy tysiącach rzędów dat zawodników.
10145. **Częściowa Regeneracja Danych w tle [Slavia-frontend]** - Ekran admina nie wymaga manualnego naciskania ikony rze-wczytania – używając okablowanego mechanizmu SWR w Nuxcie, cicho raz na 10 min wywołuje odświeżenie panelu by podać najświeższe wyniki ze zjawisk sportowych.
10146. **Dodanie Cieniowania Pod Belkę App-Bar [Slavia-mobile]** - Oddzielenie estetyczne białej belki górnej z imieniem od reszty białej części widoku aplikacji przez dodanie drobnego cienia podwyższającego kontrast na styku z kartami treningu.
10147. **Logowanie Błędów Wyjątków Wegetujących w Vue [Slavia-frontend]** - Uruchomienie globalnego Handlera we frameworku przechwytującego wszystkie niezidentyfikowane promisy asynchroniczne i prezentującego je cicho administratorowi zamiast wyrzucać straszącą białą planszę na monitor trenera.
10148. **Formatowanie Kodów SMS / Tokenów 2FA [Slavia-mobile]** - Przepisanie pinu walidującego na estetyczne ułożenie kratek (np. 6 równych pól text-input), z inteligentnym auto-focus'em przerzucającym kursor na następną formatkę.
10149. **Właściwe Zabezpieczenia Typowania na Tabelach [Slavia-backend]** - Zablokowanie tworzenia bezsensownych rzędów w relacjach tabel sportowca i ćwiczeń w SQL, poprzez wprowadzenie potężnych Foreign Keys w Sqlx, chroniących spójność podczas ewentualnych ręcznych czyszczeń skryptami serwerowymi.
10150. **Optymalizacja Nawigacji Kontekstowej SiteNav [Slavia-frontend]** - Układ pliku `SiteNav.vue` uzupełniony o skróty pod "Alt + M" wywołujące zwijaną szufladę bocznego ekranu niezależnie gdzie trener akurat przeskrolował swoją pracę.
