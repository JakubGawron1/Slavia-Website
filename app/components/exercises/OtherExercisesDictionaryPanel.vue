...
async function addExercise() {
  ...
  try {
    ...
    toast.add({ title: 'Ćwiczenie dodane', color: 'success' })
...
<template>
  <div class="space-y-6">
    <UAlert
      ...
      variant="subtle"
      icon="i-lucide-info"
      title="Słownik współdzielony"
      description="Te same ćwiczenia pojawiają się w planach treningowych, zgłoszeniach zawodników i rankingu siłowym. Dodaj tu pozycje spoza dwuboju (przysiad, wycisk, martwy, akcesoria)."
    />

    <PanelPageSection title="Wyszukiwanie" icon="i-lucide-search">
    ...
    <PanelPageSection title="Baza ćwiczeń" icon="i-lucide-dumbbell">
      <div v-if="loading" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <div v-for="n in 6" :key="n" class="h-44 animate-pulse rounded-2xl bg-muted/25" />
      </div>
      ...
      >
        ...
        <UButton v-else variant="ghost" color="neutral" @click="searchQuery = ''">
      ...
      <div v-else class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        >
          ...
          <div>
            ...
            <p v-if="e.description" class="mt-2 line-clamp-3 text-sm leading-relaxed text-muted">
              {{ e.description }}
            </p>
          </div>
      ...
      <template #body>
        <div class="space-y-5 py-2">
          <p class="text-sm text-muted">
            Wpis trafi do planów treningowych i listy ćwiczeń w rankingu siłowym.
          </p>
          ...
          <UFormField label="Kategoria" help="Pomaga w filtrowaniu listy">
            <UInput
              ...
              placeholder="np. Siła, Akcesoria, Core"
              size="lg"
              icon="i-lucide-tag"
            />
          </UFormField>
          ...
          <div class="flex flex-col gap-3 border-t border-default/50 pt-6 sm:flex-row">
            <UButton class="flex-1 justify-center" color="primary" @click="addExercise">
              Zapisz w słowniku
            </UButton>
            <UButton variant="soft" color="neutral" @click="showAddModal = false">
              Anuluj
            </UButton>
          </div>
        </div>
